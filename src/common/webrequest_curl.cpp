...
private:
    class Message
    {
        public:
            ...
            {
                ADD_SOCKET_ACTION,
                DELETE_SOCKET_ACTION,
                RESUME_POLLING,
                QUIT,
                LAST_MESSAGE_ID
            };

            explicit Message(MessageId i = LAST_MESSAGE_ID, wxSOCKET_T s = -1,
                             int f = 0)
            {
                m_messageId = i;
                m_socket = s;
...
bool SelectSocketPoller::StartPolling(wxSOCKET_T s, int flags)
{
    ...
    Message msg(Message::ADD_SOCKET_ACTION, s, flags);
    PostAndSignal(msg);
...
{
    Message msg(Message::DELETE_SOCKET_ACTION, s);
    PostAndSignal(msg);
...
{
    Message msg(Message::RESUME_POLLING, s);
    PostAndSignal(msg);
}
...
wxThread::ExitCode SelectSocketPoller::Entry()
{
    ...
    while ( !done )
    {
        ...
        while ( er == wxMSGQUEUE_NO_ERROR )
        {
            ...
            else if ( id == Message::ADD_SOCKET_ACTION )
            {
                wxSOCKET_T s = msg.GetSocket();
                int f = msg.GetFlags();
                ThreadSetSocketAction(s, f);
            }
            else if ( id == Message::DELETE_SOCKET_ACTION )
            {
                wxSOCKET_T s = msg.GetSocket();
                ThreadDeleteSocketAction(s);
            }
            else if ( id == Message::RESUME_POLLING )
            {
                wxSOCKET_T s = msg.GetSocket();
